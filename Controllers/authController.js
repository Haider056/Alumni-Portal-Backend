...
class AuthController {
  async signup(req, res) {
    try {
      const { name, email, password, country, cnic, mobile_number, city, role } = req.body;
      ...
      const userId = await userModel.createUser(name, email, hashedPassword, country, cnic, mobile_number, city, role);
  ...
  async login(req, res) {
    try {
      ...
      res.status(200).json({ message: 'Login successful', name: user.name ,email,role:userRole});
